...
import numpy as np
# from .simple_zeroshot_opt import gradient_field_gen
  ...
  if isinstance(sde, sde_lib.VPSDE) or isinstance(sde, sde_lib.subVPSDE):
    def score_fn(x, t, condition, mask):
      ...
      if continuous or isinstance(sde, sde_lib.subVPSDE):
        ...
        # first get gradient
        # joint_gradient = gradient_field_gen(x,condition,K)
        # new_x = x+joint_gradient
...
def mean_cov(x):
  ...
  cov = np.identity(m.shape[0])
  # cov = np.cov(x.transpose(1,0))
  return m,cov

def mahalanobis(m=None,cov=None,x=None):
  
  
  #the first row here is removed to avoid possible singular cov matrix
  ...
  for i in range(x.shape[0]):
    ...
    x_mu = x[i].reshape(x[i].shape[0],-1) - m
    
   
    cov = np.identity(x_mu.shape[-1])
    inv_covmat = np.linalg.inv(cov)
    left = np.dot(x_mu, inv_covmat)
    mahal = np.dot(left, x_mu.T)
    res.append(mahal.diagonal())
