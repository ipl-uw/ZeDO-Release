...
def get_pc_sampler(sde, shape, predictor, corrector, inverse_scaler, snr,
  ...
  def pc_sampler(model, condition, gradient=None,denoise_x=None, t=None,t_step=None, args=None):
    ...
    with torch.no_grad():
      ...
      if t_step <50:
        # bl = abs(gradient)>gradient.median()
      ...
      #   CosineAnnealingLR
        ...
        # vec_t[torch.any(torch.any(bl,1),1)] = 1.0
      ...
      trajs.append(x.cpu().numpy())
        ...
        # if args is not None and args.task in ['comp3d', 'comp2d']:--cluster 1  --save den --t 0.1
        #   masked_data_mean, std = sde.marginal_prob(condition, vec_t)
        #   masked_data = masked_data_mean + torch.randn_like(x) * std[:, None, None]
